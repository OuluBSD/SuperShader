#!/usr/bin/env python3
'''
Input Handling Module with Interface Definition
Extracted from common game/interactive patterns in shader analysis
Pattern frequency: 461 occurrences
'''
...
INTERFACE = {
    'inputs': [
        ...
        {'name': 'hoverArea', 'type': 'vec4', 'direction': 'uniform', 'semantic': 'interactive_area'},
        {'name': 'time', 'type': 'float', 'direction': 'uniform', 'semantic': 'time_parameter'}
    ],
    'outputs': [
        {'name': 'isHovered', 'type': 'float', 'direction': 'out', 'semantic': 'hover_status'},
        {'name': 'isClicked', 'type': 'float', 'direction': 'out', 'semantic': 'click_status'},
        {'name': 'highlight', 'type': 'float', 'direction': 'out', 'semantic': 'highlight_intensity'}
    ],
    'uniforms': [
        {'name': 'mousePos', 'type': 'vec2', 'semantic': 'mouse_position'},
        {'name': 'clickState', 'type': 'float', 'semantic': 'click_status'},
        {'name': 'hoverArea', 'type': 'vec4', 'semantic': 'interactive_area'},
...
def get_pseudocode():
    '''Return the pseudocode for this game/interactive module'''
    return pseudocode
...
def get_metadata():
    ...
    return {
        ...
        'dependencies': [],
        'conflicts': [],
        'description': 'Input handling and interactive element visualization',
        'interface': INTERFACE
    }