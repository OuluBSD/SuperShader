...
def test_game_conflict_detection():
    ...
    try:
        ...
        minimalist_branch = hud_branches.get('minimalist')
        if not minimalist_branch:
            print("✗ No minimalist HUD branch found")
            return False
            
        minimalist_conflicts = minimalist_branch.get('conflicts', [])
        ...
        # Check that themed HUD conflicts with others
        themed_branch = hud_branches.get('themed')
        if not themed_branch:
            print("✗ No themed HUD branch found")
            return False
            
        themed_conflicts = themed_branch.get('conflicts', [])
        ...
        click_conflicts = click_branch.get('conflicts', [])
        ...
        subtle_conflicts = subtle_branch.get('conflicts', [])
