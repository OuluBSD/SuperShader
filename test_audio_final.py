#!/usr/bin/env python3
"""
Final test script for audio modules with management code
Tests the integration of audio modules with the management system
"""
...
def test_audio_conflict_detection():
    ...
    try:
        ...
        fourier_branch = spectrum_branches.get('fourier', {})
        if not fourier_branch:
            print("✗ No fourier branch found")
            return False
            
        fourier_conflicts = fourier_branch.get('conflicts', [])
        ...
        # Check that wavelet conflicts with others
        wavelet_branch = spectrum_branches.get('wavelet', {})
        if not wavelet_branch:
            print("✗ No wavelet branch found")
            return False
            
        wavelet_conflicts = wavelet_branch.get('conflicts', [])
        expected_wavelet_conflicts = ['fourier', 'autocorrelation']
        
        if all(conflict in wavelet_conflicts for conflict in expected_wavelet_conflicts):
            print("✓ Wavelet transform properly conflicts with fourier and autocorrelation")
        else:
            print(f"✗ Wavelet transform conflicts {wavelet_conflicts} don't match expected {expected_wavelet_conflicts}")
            return False
            
        # Check that autocorrelation conflicts with others
        autocorr_branch = spectrum_branches.get('autocorrelation', {})
        if not autocorr_branch:
            print("✗ No autocorrelation branch found")
            return False
            
        autocorr_conflicts = autocorr_branch.get('conflicts', [])
        expected_autocorr_conflicts = ['fourier', 'wavelet']
        
        if all(conflict in autocorr_conflicts for conflict in expected_autocorr_conflicts):
            print("✓ Autocorrelation properly conflicts with fourier and wavelet")
        else:
            print(f"✗ Autocorrelation conflicts {autocorr_conflicts} don't match expected {expected_autocorr_conflicts}")
            return False
        ...
        additive_branch = synth_methods.get('additive', {})
        if not additive_branch:
            print("✗ No additive branch found")
            return False
            
        additive_conflicts = additive_branch.get('conflicts', [])
        ...
        low_pass_branch = filter_branches.get('low_pass', {})
        if not low_pass_branch:
            print("✗ No low_pass branch found")
            return False
            
        lp_conflicts = low_pass_branch.get('conflicts', [])
